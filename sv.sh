...
echo "SSH: ${psshd:=/usr/sbin/sshd}" >&2
...
read -r vhkey
echo '' >&2
vhkey="`echo "$vhkey" | grep '^[^#].*$' | sed '/^==$/d' | sed 's,^ *\([^ ]\+\) *$,-h \1,'`" 
...
echo "cd ${rbase:-.} && ${psshd:-/usr/sbin/sshd} ${dssopts:--D} ${optsf} ${vhkey} ${optsl} -p ${vport:-22}" | sed 's, \+, ,g'

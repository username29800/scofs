...
# ssh noninteractive session opener
...
echo "cd ${rbase:-.} && ${pssh:-/usr/bin/ssh} ${dssopts:--C} ${optcf} ${pkeypriv} ${rjmp} ${rlfd} ${rrfd} ${rdfd} ${optcl} -${portf:=p} ${vport:-22} ${vuser}@${vhost}" | sed 's, \+, ,g'

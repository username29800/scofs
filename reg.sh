...
echo "cd ${rbase:-.} && ${pssh:-/usr/bin/ssh} ${dssopts:--C} ${optcf} ${pkeypriv} ${rjmp} ${rlfd} ${rrfd} ${rdfd} ${optcl} -p ${vport:-22} ${vuser}@${vhost} "\""mkdir -p ${2:-.ssh} && chmod 0700 ${2:-.ssh} && echo '"`cat $lkeypub`"' >> ${2:-.ssh}/${3:-authorized_keys} && chmod 0600 ${2:-.ssh}/${3:-authorized_keys}"\""" | sed 's, \+, ,g'
